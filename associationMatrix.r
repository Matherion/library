...
associationMatrix <- function(dat, x, y=NULL, conf.level = .95, correction = "fdr",
  ...
  ### confidence interval, digits etc, and create matrices for storign
  ### results
  res <- list();
  ...
  res$es <- matrix(nrow = length(x), ncol = length(y));
  res$statistic <- matrix(nrow = length(x), ncol = length(y));
  ...
  res$es.type <- matrix(nrow = length(x), ncol = length(y));
  
  xCounter <- 1;
  for(curXvar in x) {
    yCounter <- 1;
    for(curYvar in y) {
      if (measurementLevelsX[xCounter] == "dichotomous") {
        if (measurementLevelsY[yCounter] == "dichotomous") {
          ### Odds ratio
          
        }
        else if (measurementLevelsY[yCounter] == "nominal") {
          ### Cramer's V
          
        }
        else if (measurementLevelsY[yCounter] == "ordinal") {
          ### Cramer's V
          
        }
        else if (measurementLevelsY[yCounter] == "numeric") {
          ### Cohen's d
          
        }        
      }
      else if (measurementLevelsX[xCounter] == "nominal") {
        if (measurementLevelsY[yCounter] == "dichotomous") {
          ### Cramer's V
      ...
      else if (measurementLevelsX[xCounter] == "ordinal") {
        if (measurementLevelsY[yCounter] == "dichotomous") {
          ### Cramer's V
      ...
      else if (measurementLevelsX[xCounter] == "numeric") {
        if (measurementLevelsY[yCounter] == "dichotomous") {
          ### Cohen's d
          curTest <- meanDiff(dat[,curXvar] ~ dat[,curYvar], conf.level = conf.level);
          res$estimate <- curTest$meanDiff.d;

          res$statistic[xCounter, yCounter] <- curTest$estimate;
          ...
          res$estimate.type[xCounter, yCounter] <- "d";
          ...
          res$estimate[xCounter, yCounter] <- curTest$estimate;
          res$statistic[xCounter, yCounter] <- curTest$statistic;
          res$parameter[xCounter, yCounter] <- curTest$parameter;
          res$ci.lo[xCounter, yCounter] <- curTest$conf.int[1];
          res$ci.hi[xCounter, yCounter] <- curTest$conf.int[2];
          res$p.raw[xCounter, yCounter] <- curTest$p.value;
          res$estimate.type[xCounter, yCounter] <- "r";
        }        
      }
      yCounter <- yCounter + 1;
    }
